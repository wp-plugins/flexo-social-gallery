...
//echo'<pre>';print_r($params);echo'</pre>';
		...
		$razmer = count($images);
		$ret = '<div><a href='.$permalink.'> '.$params['link'].' </а></div>';
		$ret .='<div id="part_g">';
		$ret .= "<a name='".$link_name."'></a>";
		
		
			if ($params['br_pic']%2	== 0){
				$k=0;
				$br_m=($params['br_pic'])/2;
			}//4etni
			else {
				$k=1;
				$br_m=($params['br_pic']-1)/2;			
			}//ne4etni
			
			
		$br_g= round($params['br_pic']/2);

	for ($i=0; $i < $razmer; $i++) {
		if ($images[$i]->pid == $pid){
			
			if($i >= $razmer-$br_g){
				for($m=$razmer-$params['br_pic'];$m < $razmer; $m++){
					if ($images[$m]->pid==$pid){
					$ret.= '<a href="'.self::img_url($permalink,$pid,$gallery_anchor).'"class="selected"><img src="'.$images[$m]->thumbURL.'" /></a>';
					}
					else{
						$ret.= $images[$m]->url;
					}
			...
			if($i < $razmer-$br_g) {
				for ($j = $i-$br_m; $j < $i+$br_m+$k; $j++ )
				{
					if ($j == -$br_m-$k){
							
							for($m = 0;$m < $params['br_pic']; $m++) {
								//$ret.= $images[$m]->url;
					...
					if ($j >= 0 && $j < $razmer ){ 
							if ($images[$j]->pid==$pid){
							$ret.= '<a href="'.self::img_url($permalink,$pid,$gallery_anchor).'" class="selected"><img src="'.$images[$j]->thumbURL.'" /></a>';
							}
							else{
							$ret.= $images[$j]->url;
							}
	...
	public static function gallery_admin_menu(){
		...
		<form method="POST">
				...
				</br>Link (back to gallery)<input type="text" name="link" value="<?php echo $link; ?>"/>
			
				</br>Gallery ID<input style="width: 100px;" type="text" name="GalleryID" value="<?php echo $GalleryID; ?>" />
				
				</br>number of pictures in the thumbnail Previews<input style="width: 100px;" type="text" name="br_pic" value="<?php echo $br_pic; ?>" />
				
				</br>where to show the thumbnail Previews		
							<input type="radio" name="nav" value="up" <?php echo $nav == 'up' ? 'checked' : ''; ?>/>up
							<input type="radio" name="nav" value="down" <?php echo $nav == 'down' ? 'checked' : ''; ?>/>down
							<input type="radio" name="nav" value="without" <?php echo $nav == 'without' ? 'checked' : ''; ?>/>without
				</br></br><input type="submit" name="submit" value="Generating"></br>
				
				
				</br>Generating code<input  type="text" style=" width:400px;" name="gen_text" value= " <?php
							echo "[flexogallery ";
	...
	public static function gallery_menu () {
		add_options_page('flexo gallery','Flexo Social Gallery','manage_options','fl-gallery','FlexoSocialGallery::gallery_admin_menu','');
	
	}
		
}//class
if(function_exists('add_action')):
	add_action('admin_menu', 'FlexoSocialGallery::gallery_menu');
	if (!is_admin()  ) {
...
// izkarva 5-te snimki za navigaciq
	...
	public static function gallery_admin_menu(){
		...
		<form method="POST">
				...
				</br>Generating code<input  type="text" style=" width:300px;" name="gen_text" value= " <?php
							echo "[flexogallery ";
							echo " GalleryID='".$GalleryID."' ";
							echo "nav='";
							echo $nav."' ";
	...
	public static function gallery_menu () {
		add_options_page('flexo gallery','flexo gallery','manage_options','fl-gallery','FlexoSocialGallery::gallery_admin_menu','');
