...
Description: Facebook Social Gallery
Version: 1.0004 
Stable tag:1.0004
...
// izkarva 5-te snimki za navigaciq
	public static function show_part_gallery($pid,$params) {
		...
		$razmer = count($images);
		$ret = '<div><a href='.$permalink.'> '.get_option(fg_link).' </а></div>';
		$ret .='<div id="part_g">';
	...
	public static function gallery_admin_menu(){
		...
		if ($_POST['submit']){
				...
				update_option(fg_link, $link);
		}
		
		?>
		<form method="POST">
			
				</br>Link (back to gallery)<input type="text" name="link" value="<?php echo get_option(fg_link); ?>"/>
...
if(function_exists('add_action')):
	add_option('fg_link','comments','fg_link');
