...
//echo $pics_html;
		if ($check=='true')
				return '<div class="all_gallery">'.$pics_html.'</div>';
		else
				return $new_result;
	}
	...
	public static function filter($content) {
		...
		if($is_image){
				...
				if(class_exists('flexoFBManager')){
					...
					$like 		=	flexoFBManager::get_form($pid,'likes-002',$url);
					$comments =	flexoFBManager::get_form($pid,'all-001',$url);
				}
			
				if ($params['nav']=='up'){
					$ret .= $gallery.$image.$like.$comments;
				}
				if ($params['nav']=='down'){
					$ret .= $image.$gallery.$like.$comments;
				}
				if ($params['nav']=='without'){
					$ret .= $image.$like.$comments;
				}
		}
	...
	public static function format_image($pid,$arg) {
		...
		for ($i=0; $i < $razmer; $i++) {
			...
			<?php
				...
				$ret.= 	'<div id="pic"><a href="'.$permalink.'?'.self::key_img.'='.$arg[$br+1]->pid.$gallery_anchor .'"><img class="social-gallery" src="'.$dirresult[0]->path.'/'.$arg[$nom]->filename .'" ></a></div>';
